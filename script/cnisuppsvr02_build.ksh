...
#                                                                                         #
# 6. ./script/cnisuppsvr02_build.ksh 2>&1 | tee ./installation.log                        #
###########################################################################################
...
config_pg()
{
 ...
 sudo -u postgres echo "create user $PG_USER with password '$PG_PASS';" > ./pg_setup.sql
 sudo -u postgres echo "create database $PG_DBNAME owner $PG_USER;" >> ./pg_setup.sql
 sudo -u postgres echo "alter database $PG_DBNAME set datestyle to 'sql,european';" >> ./pg_setup.sql
 sudo -u postgres psql -f ./pg_setup.sql
 sudo -u rm -f ./pg_setup.sql
 sleep 2
...
config_fw_sec_httpd()
{
 ...
 iptables -I INPUT 4 -p tcp --dport $APP_PORT -j ACCEPT
 iptables -L
